 ...
 * Completes entry/exit symmetry: entry has ValidationService, exit has
...
public final class ExitQualificationService {
     ...
     *
     * @param exitSignalDirection Direction from exit signal (BUY signal = close
     *                            LONG)
    ...
    public ExitQualificationResult qualify(
        ...
        if (!isDirectionValid(trade, exitDirection)) {
            builder.addError("DIRECTION_MISMATCH",
                    String.format("Exit direction %s is not opposite to trade direction %s",
        ...
        if (userContext.portfolioPaused()) {
            builder.addError("PORTFOLIO_PAUSED", "Portfolio is paused");
            return builder.build();
        }
     ...
     */
    private Direction getExitDirection(Direction signalDirection) {
        return signalDirection == Direction.BUY ? Direction.SELL : Direction.BUY;
    }
    ...
    private boolean isDirectionValid(Trade trade, Direction exitDirection) {
        ...
        Direction tradeDirection = Direction.valueOf(trade.direction());
        return (tradeDirection == Direction.BUY && exitDirection == Direction.SELL)
                || (tradeDirection == Direction.SELL && exitDirection == Direction.BUY);
    }
     ...
     */
    private boolean isExitWindowOpen(ExitReason exitReason) {
        LocalTime now = LocalTime.now(ZoneId.of("Asia/Kolkata"));

        // Stop loss exits allowed anytime during market hours
        if (exitReason == ExitReason.STOP_LOSS || exitReason == ExitReason.TRAILING_STOP) {
     ...
     */
    private String determineExitOrderType(ExitReason exitReason) {
     ...
     */
    private BigDecimal calculateExitLimitPrice(ExitReason exitReason, BigDecimal exitPrice, Direction exitDirection) {
        // For target hits, use detected price as limit
        // For time-based, give slight buffer
        if (exitReason == ExitReason.TARGET_HIT) {
            return exitPrice;
        }

        if (exitReason == ExitReason.TIME_BASED) {
            // Give 0.1% buffer for fills
            BigDecimal buffer = exitPrice.multiply(new BigDecimal("0.001"));
            String directionStr = exitDirection.toString(); // Convert Direction enum to String
            return Direction.valueOf(directionStr) == Direction.SELL
