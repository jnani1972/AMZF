...
public final class TradeManagementServiceImpl implements TradeManagementService {
    ...
    public Trade createTradeForIntent(TradeIntent intent, Signal signal) {
        ...
        TradeClassifier.TradeClassification classification = TradeClassifier.classify(intent.userId(), signal.symbol(),
                existingTrades);
    ...
    private void placeEntryOrder(Trade trade, TradeIntent intent, Signal signal) {
        ...
        // Build order request
        BrokerAdapter.BrokerOrderRequest orderRequest = buildEntryOrderRequest(intent, signal);
     ...
     */
    private BrokerAdapter.BrokerOrderRequest buildEntryOrderRequest(TradeIntent intent, Signal signal) {
    ...
    @Override
    public void closeTradeOnExitFill(String tradeId, BigDecimal exitPrice, Integer exitQty, String exitReason,
    ...
    private BigDecimal calculateLogReturn(Trade trade, BigDecimal exitPrice) {
        ...
        if ("BUY".equals(trade.direction())) {
            return BigDecimal.valueOf(
                    Math.log(exitPrice.divide(trade.entryPrice(), 6, java.math.RoundingMode.HALF_UP).doubleValue()));
        } else {
            return BigDecimal.valueOf(
                    Math.log(trade.entryPrice().divide(exitPrice, 6, java.math.RoundingMode.HALF_UP).doubleValue()));
        }
    }
