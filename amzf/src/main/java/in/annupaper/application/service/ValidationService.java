...
import in.annupaper.domain.model.ValidationErrorCode;
import in.annupaper.domain.model.ValidationResult;
...
import in.annupaper.application.service.PositionSizingService;
...
public final class ValidationService {
    ...
    private BigDecimal getConfluenceMultiplier(BigDecimal score) {
        ...
        if (s <= 0.25)
            return new BigDecimal("1.2"); // VERY_STRONG
        if (s <= 0.45)
            return BigDecimal.ONE; // STRONG
        if (s <= 0.65)
            return new BigDecimal("0.8"); // MODERATE
        if (s <= 0.85)
            return new BigDecimal("0.6"); // WEAK
    ...
    private String determineProductType(UserBroker userBroker, Signal signal) {
        ...
        if (userBroker.allowedProducts() != null && !userBroker.allowedProducts().isEmpty()) {
            ...
            if (userBroker.allowedProducts().contains("MIS"))
                return "MIS";
            if (userBroker.allowedProducts().contains("CNC"))
                return "CNC";
