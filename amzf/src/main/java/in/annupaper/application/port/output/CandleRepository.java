...
public interface CandleRepository {
     ...
     */
    default void upsertBatch(List<HistoricalCandle> candles) {
        insertBatch(candles);
    }
     ...
     */
    List<HistoricalCandle> findBySymbolAndTimeframe(String symbol, TimeframeType timeframe, Instant from, Instant to);

    /**
     * Find the latest candle for a symbol and timeframe.
     */
    HistoricalCandle findLatest(String symbol, TimeframeType timeframe);

    /**
     * Find all candles for a symbol and timeframe (ordered by timestamp desc).
     */
    List<HistoricalCandle> findAll(String symbol, TimeframeType timeframe, int limit);
