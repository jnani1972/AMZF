...
public final class PostgresInstrumentRepository implements InstrumentRepository {
    ...
    @Override
    public void saveInstruments(String brokerId, List<BrokerAdapter.Instrument> instruments) {
        String sql = """
            INSERT INTO instruments (broker_id, exchange, trading_symbol, name, instrument_type, segment, token, lot_size, tick_size)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            for (BrokerAdapter.Instrument inst : instruments) {
                ...
                ps.setString(6, inst.segment());
                ps.setString(7, inst.token());
                ps.setInt(8, inst.lotSize());
                ps.setBigDecimal(9, inst.tickSize());
                ...
                if (count % 1000 == 0) {
                    ps.executeBatch();
                    conn.commit();
                    log.debug("Saved {} instruments for {}", count, brokerId);
                }
            }

            ps.executeBatch();
            conn.commit();
            log.info("Saved total {} instruments for {}", count, brokerId);
