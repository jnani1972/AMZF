...
import in.annupaper.domain.signal.ConfluenceStrength;
import in.annupaper.domain.signal.ConfluenceType;
...
public record MTFConfig(
    ...
    public static MTFConfig from(TimeframeAnalysis htf, TimeframeAnalysis itf, 
        ...
        int buyZoneCount = 0;
        if (htf.inBuyZone()) buyZoneCount++;
        if (itf.inBuyZone()) buyZoneCount++;
        if (ltf.inBuyZone()) buyZoneCount++;
    ...
    public BigDecimal calculatePWin(BigDecimal price) {
        ...
        if (totalDist.compareTo(BigDecimal.ZERO) == 0) return BigDecimal.ZERO;
